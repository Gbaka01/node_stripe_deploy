...
const register = async (req, res) => {
  try {
    ...
    // 🔐 Hash fait automatiquement via le hook mongoose
    const user = new User(body);
    await user.save();

    // 🛒 Créer un panier vide "pending" automatiquement
    await Command.create({
      user: user._id,
      status: "pending",  // ✔ string valide
      items: [],
      totalAmount: 0
    });
...
const login = async(req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      token: jwt.sign(
        { id: user._id, email: user.email, address: user.address, zipcode: user.zipcode, town: user.town },
